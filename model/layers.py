...
class SpeakerEncoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.lstm = nn.LSTM(hparams.n_mel_channels + hparams.E, int(hparams.speaker_encoder_hidden_dim / 2), 
    ...
    def forward(self, x, input_lengths, text, text_lengths):
        ...
        mask = self.get_mask(text, text_lengths)
        # text =  text.transpose(1,2)
        ...
        while len(contexts)< mels.size(0):
            mel = mels[len(contexts)]
            # out , scores = self.stl(mel, text)
            # (output, score)  = self.se_alignment(mask, mel, text)
            (output, score)  = self.se_alignment(text, mel, mask)
            contexts += [output]
            scores += [score]
        # pdb.set_trace()
...
class VanillaAttention(nn.Module):
    ...
    def __init__(self, hparams,
        ...
        self.query_dim = hparams.n_mel_channels # q
        self.num_hidden = hparams.E # h
        self.embedding_dim = hparams.encoder_embedding_dim # e
    ...
    def forward(self, input_sequence, query_vector, mask):
        ...
        pdb.set_trace()
        if self.additive:
            s, x1 = additive_fn(x=input_sequence, q=query_vector, linear_x=self.linear_x,
