...
def get_files(path: Union[str, Path], extension='.npy'):
    ...
    wav = list(path.rglob(f'*{extension}'))
...
def preprocess_wav(fpath_or_wav: Union[str, Path, np.ndarray], source_sr: Optional[int]=None):
    ...
    # save wav back to the disk
    os.remove(fpath_or_wav)
    librosa.output.write_wav(fpath_or_wav, wav, sampling_rate)
...
def preprocess(root):
    ...
    pool = Pool(cpu_count())
    pool.map(preprocess_wav,abs_paths)
...
# pdb.set_trace()
# wav = get_files('/home/hk/voice_conversion/nonparaSeq2seqVC_text-dependent_SE/reader/VCTK',extension='.wav')

# preprocess_wav(wav, source_sr)