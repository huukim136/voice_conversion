...
class TextMelIDLoader(torch.utils.data.Dataset):
    ...
    # def __init__(self, list_file, shuffle=True):
        ...
        mel_mean = np.float32(np.load('/home/hk/voice_conversion/nonparaSeq2seqVC_text-dependent_SE/reader/spk_embeddings/mean_unseen1.npy'))
        self.mel_mean1 = mel_mean[:, None]
        mel_std = np.float32(np.load('/home/hk/voice_conversion/nonparaSeq2seqVC_text-dependent_SE/reader/spk_embeddings/std_unseen1.npy'))
        self.mel_std1 = mel_std[:,None]
    ...
    def get_text_mel_id_pair(self, path):
        ...
        text_input = torch.tensor([1,1,1])
        # print("text_input ", text_input)
        mel = np.load(mel_path)

        if (mel_path == '/home/hk/voice_conversion/nonparaSeq2seqVC_text-dependent_SE/reader/unseen/QOAF3CHR065.npy'):
        
            mel = (mel - self.mel_mean1)/ self.mel_std1
        else:
